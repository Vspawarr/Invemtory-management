...
- **`Motherson_Holding_Cost_Model.xlsx`** — the same model as an Excel workbook,
  ...
  *Holding Cost Model* (all formulas, nothing hardcoded).
...
1. **Frame the number, don't promise it.** 15–20% is a credible target *once
   ...
   air.
2. **Four levers, not one silver bullet:**
...
3. **Ask for the data checklist live** (in the dashboard, section 7): SKU
...
4. **Anchor credibility, not precision.** Every figure shown is explicitly
